...
const PostActionButtons = ({ id, ws_id }) => {
  ...
  return (
    <PostActionButtonBlock>
      <Link to={`${id}/update`}>
        <ActionButton style={{backgroundColor:'#7986CB', color:'white'}}>수정</ActionButton>
      </Link>
      &nbsp;
      <ActionButton onClick={deletePost} style={{backgroundColor:'#7986CB', color:'white'}}>삭제</ActionButton>
    </PostActionButtonBlock>
  );
};
